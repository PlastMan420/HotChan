...
builder.Services.AddIdentity<IdentityUser, IdentityRole>().AddEntityFrameworkStores<HotChanContext>();
...
app.MapGet("/post/{postId}", [AllowAnonymous] async (Guid postId, [FromServices]PostRepostiry postRepo) =>
{
...
app.MapGet("/post/catalog", [AllowAnonymous] async ([FromServices] PostRepostiry postRepo) =>
{
...
app.MapPost("/file/new", async ([FromServices] IFormFile file,[FromServices]PostRepostiry postRepo, HttpRequest request) =>
{
    var newPost = await request.ReadFromJsonAsync<PostDialogueDto>();
    if (newPost == null)
        return Results.BadRequest();
